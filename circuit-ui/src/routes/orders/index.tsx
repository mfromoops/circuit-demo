    ...
    return (
        <div class="mx-5 pb-5">
            <div class="flex justify-between">
                <h1>Orders</h1>
                <button onClick$={() => createPlan.submit({orders: Array.from(orders.value)})}>
                    Create Plan
                </button>
            </div>
        {Array.from(orders.value.keys()).map(key => {
            const order = orders.value.get(key)![0];
            const storeOrders = orders.value.get(key);
            return (
                <>

                <h1 class="text-xl my-5">
                    {order.order.store_id.name} 
                </h1>
                <h3>
                    {order.order.store_id.address}
                </h3>
